...
class SurveyAdmin extends Survey_Common_Action
{
    ...
    public function delete($iSurveyID)
    {
        ...
        if (Permission::model()->hasSurveyPermission($iSurveyID, 'survey', 'delete'))
        {
            if (Yii::app()->request->getPost("delete") == 'yes' && !empty($_POST['justification']))
            {
                 ...
                 * Número de lineas: 28
                 * Se captura la fecha del sistema, la justificación, el usuario logueado, se consulta
                 * el titulo de la encuesta y su dueño para luego adicionar estos campos a la tabla
                 * que contiene los registros de las encuestas eliminadas
                 * -------------------------------------------------------------------------------------
                 */
                $survey = Survey::model()->findByPk($iSurveyID);
                if (!is_null($survey)) {
    ...
    */
    ...
    public function templateconfiguration(){
        ...
        $this->_renderWrappedTemplate('survey', 'configurationTemplate_view', $aData);
    }
